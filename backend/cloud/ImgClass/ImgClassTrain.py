...
class QuietProgressCallback(tf.keras.callbacks.Callback):
    """Custom callback for quiet training progress."""
    
    def on_epoch_end(self, epoch, logs=None):
        """Print epoch results without verbose output."""
        logs = logs or {}
...
class ImgClassTrainer:
    ...
    def train_stage1(self):
        ...
        callbacks_stage1 = [
            QuietProgressCallback(),  # Custom quiet progress callback
            tf.keras.callbacks.EarlyStopping(
                monitor="val_loss",
                patience=int(self.initial_epochs*.1),
                restore_best_weights=True,
                verbose=0  # Suppress callback output
            ),
