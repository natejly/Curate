...
export function AvailableDatasetsSection() {
  ...
  async function handleTrain() {
    ...
    setSelected(null);
    // Navigate immediately so the console appears right away
    router.push(`/training-console?sessionId=${encodeURIComponent(sessionId)}`);
    // Fire-and-forget job start; pass sessionId so logs align
    fetch(`http://localhost:8000/train-s3/${encodeURIComponent(selected)}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ session_id: sessionId })
    }).catch(() => {
      // Non-blocking; console will show fallback messages if job fails to start
    });
  }
